...
class Log_Linear_Model:
    ...
    def create_feature_space(self):
        ...
        self.g=np.zeros(len(self.feature_space))#梯度向量
    ...
    #iterations表示的是迭代次数，max_iterations表示多少次迭代没有提升则退出，eta指的是学习率，batch_size指的是批量次数
    def SGD_Training(self,iterations,max_iterations,eta,batch_size,shuffle=False):
            ...
            for j in range(len(train_sentences)):
                ...
                for i in range(len(sentence)):
                    ...
                    if(batch_size==b):
                        ...
                        b=0
                        self.g=np.zeros(len(self.feature_space))
            if(b>0):
                self.weight+=self.g
                b=0
                self.g=np.zeros(len(self.feature_space))
