...
class Log_Linear_Model:
    ...
    def predict(self,sentence,position):
        ...
        for tag in self.tag_list:
            ...
            score.append(self.calculate_score(template))
    ...
    #iterations表示的是迭代次数，max_iterations表示多少次迭代没有提升则退出，eta指的是学习率，batch_size指的是批量次数,C指的是正则项系数，decay指的是学习率的衰退系数，对于这次优化，在正则项方面使用C作为正则项系数
        ...
        for m in range(iterations):
            ...
            for j in range(len(train_sentences)):
                ...
                for i in range(len(sentence)):
                    #实现算法第7行这一公式
                    correct_tag=tags[i]
                    correct_feature_template=self.create_feature_template(sentence,i,correct_tag)
                    for f in correct_feature_template:
                        if(f in self.feature_space):
                            self.g[self.feature_space[f]]+=1
                    all_feature_lst=[self.create_feature_template(sentence,i,ev_tag) for ev_tag in self.tag_list]
                    all_score_lst=[self.calculate_score(template) for template in all_feature_lst]
                    sum_score=logsumexp(all_score_lst)#先求它的exp，然后求和，再求和的log
                    temp_result=np.array(all_score_lst)-sum_score
                    for template,score in zip(all_feature_lst,temp_result):
                        for f in template:
                            if(f in self.feature_space):
                                self.g[self.feature_space[f]]-=np.exp(score)
                    b=b+1
                    if(batch_size==b):
                        if(regularization):
...
if __name__ == '__main__':
    m=Log_Linear_Model("train.conll","dev.conll")
    m.create_feature_space()
    m.SGD_Training(50,20,0.5,50,0.0001,0.96,shuffle=True,regularization=True,Simulated_annealing=True)
